...
import { getCoSujet, updateCoSujet, type CoSujet, type EditableCoQuestion } from "@/lib/coSujets";
...
export default function AdminSujetModifier() {
      ...
      if (result) {
        ...
        setQuestions(
          result.questions.map((q) => ({
            number: q.number,
            existingAudioUrl: q.audio,
            existingImageUrl: q.image ?? null,
            newAudioFile: null,
            newImageFile: null,
            options: q.options,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const payload: EditableCoQuestion[] = questions.map((q) => ({
        number: q.number,
        existingAudioUrl: q.existingAudioUrl,
        existingImageUrl: q.existingImageUrl,
        newAudioFile: q.newAudioFile,
        newImageFile: q.newImageFile,
        options: q.options,
        correctIndex: q.correctIndex,
      }));
      await updateCoSujet(sujet.id, sujet.slug, title.trim(), isFree, payload);
      navigate("/admin");
    } catch (err) {
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-16 sm:py-24">
      <Seo title="Modifier le sujet" description="Modifier un sujet de Compréhension Orale." />
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-6">
        ...
        {questions.map((q, index) => (
          <div key={q.number} className="card-shell space-y-3 p-5">
            <h2 className="font-semibold">Question {q.number}</h2>

            <div>
              <label className="block text-xs font-semibold text-muted-foreground">
                Remplacer l'audio (laisse vide pour garder l'actuel)
              </label>
              <input
                type="file"
                accept="audio/*"
                onChange={(e) => updateQuestion(index, { newAudioFile: e.target.files?.[0] ?? null })}
            ...
            <div>
              ...
              <input
                type="file"
                accept="image/*"
                onChange={(e) => updateQuestion(index, { newImageFile: e.target.files?.[0] ?? null })}
                className="mt-1 w-full text-sm"
              />
            </div>
